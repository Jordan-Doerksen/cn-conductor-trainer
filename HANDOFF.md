...
- **Honest rough edges (polish later):** kicking is never *forced* (spotting always works) — it's the no-joint efficient line the hints point to and the joints counter rewards; could add a per-puzzle "par joints" target later. Linked from the hub as a **"Game"** card (orange LED, `.tag.g`).
