...
### 5.2.0 GP Switching Sim — REBUILD SPEC (2026-06-18)
Jordan is taking the Switching Sim into its **own standalone repo / GitHub Pages**, built ground-up. Full design + handoff in **`switching-sim-rebuild/`** (`SPEC.md`, `HANDOFF.md`, `README.md`). Two corrections drive the rebuild: (1) **score = fewest MOVES, not joints** — the prototype's joint-par produced wasteful "kick-then-pull-back" lines; joints become a secondary quality stat (Jordan's call, 2026-06-18). (2) **ramp the rules** — start simple (every car identical, no kicking) and introduce one rule at a time (securing/kicking → foul/clearance → loads/empties + "don't mix"/"no kicking loaded into empty" → car length → road jobs → outbound manifest order). Cite every rule; **this repo's CROR PDF + signals are the source of truth.** Yard-speed citation added: **Rule 105(b) — non-main ≤15 MPH, REDUCED speed** (verified). The §5.2.1 prototype below stays as the working reference but its **joint-based par is the known flaw the rebuild fixes.**

### 5.2.1 GP Switching Sim — animated conductor game (prototype, 2026-06-17)
...
- **Geometry:** a real ladder yard — LEAD + diagonal ladder + 6 body tracks (AS71 nearest the lead … AS76 at the top). Each track's route = `[A,B,junction_i,bodyEnd]` polyline; positions are arclength `s` along it; `polyAt` interpolates x/y/angle. Throat-anchored car slots. **Widened 2026-06-17: canvas `W=1340` (was 980), `RIGHT=1306` (was 946), `.wrap` max-width 1400px** — tracks fill most of the screen on desktop; purely visual (lead/ladder/junctions/cars/par all unchanged, since car positions key off the ladder not the track length).
...
- **Animation polish (2026-06-17):** (1) **no more re-randomize jump** — `loadPuzzle` snapshots the start (`startState`); Reset and Watch-optimal now `resetRun()` (restore that snapshot) instead of re-rolling offsets, so cars don't jump when you hit them. (2) **loaded-engine pulls don't overlap** — `doPull` couple target now backs in past cars already on the engine (`sC = sBody(i,0)-COUP - engine.length*CARLEN`), so a pull onto a held cut lands the new car exactly at its standing spot (verified: min car gap = CARLEN, no overlap; e.g. puzzle 4's two-track pull).
- **Variable car lengths (2026-06-17):** cars are no longer one uniform size. `carLen(label)` via `TYPE={m:74,p:60,r:74}` px — **m,r = autorack/car-carrier (~89ft), p = lumber/centerbeam (~73ft), everything else standard (CARLEN=42px ≈ 50ft)**. Only the lowercase **stray** labels are long; all uppercase puzzle cars stay standard, so par/opt/lead are untouched. This required refactoring the whole position core from `index*CARLEN` to **cumulative real lengths**: `state.offset` (slot-0 centre) → `state.head[T]` (throat car near-edge from the junction); `sumLen`/`engSlotS`/`throatS` helpers; LEAD check is now **length-based** (`LEAD_LEN=170px` ≈ 4 std); foul check is `head<FZONE`; `parkS`, doPull/doSpot/doKick, render, and the kicked-`flying` cut all lay cars out cumulatively. Adjacent car centres differ by exactly half the sum of their lengths ⇒ no overlap by construction. **Re-verified: 640 replays (40 rolls × 16) all win at par with no foul/lead failure; long cars render at correct lengths with proper spacing.**
- **Loaded/empty + kick limit (2026-06-17, CROR 113.5):** cars now have a **loaded** flag (`state.loaded[label]`, ~45% loaded, random per load) drawn as **solid = loaded / hollow = empty**. **113.5(a)(v)** in the rulebook defers the max kicked to *special instructions* ("maximum tonnage"), so the GP practical limit is encoded: **kick ≤ 5 empties, ≤ 3 if any loaded** (refused otherwise). Doesn't affect par/opt (all opt kicks ≤ 3). Labelled in the yard note + how-to + rules list, cited as a 113.5 special instruction. Verified: 6-empty & 4-with-loaded refused, 5-empty allowed; 800-replay harness still clean.
- **Sound + mute (2026-06-17):** synthesized Web Audio (no files) — coupling clunk, kick roll (noise+sweep), points click, win chime, refusal buzz, **train horn**; lazy `AudioContext` resumed on first gesture. 🔊/🔇 toggle in the header, persisted in `localStorage` (`gpss_mute`). All sfx verified to fire without error.
- **Inbound / outbound — road jobs (2026-06-17, Phase 1):** new puzzle type + tier. A puzzle with `inbound:{cars,to}` plays an **arrival animation** on load (`arrive()`): the cut rolls in from off-screen-left down the lead (polyAt now **extrapolates s<0**), sets out on `to` + ties down, engine pulls clear. Goal with `depart:true` triggers a **departure animation** on win (`departTrain()` from `checkWin`): the engine backs onto the assembled outbound, couples, and pulls it off-screen-left → "Train away." Loaded/empty is **deterministic** when a puzzle gives `loads:[...]` (else random) so outbound goals are well-defined. Phase-1 intro puzzles: **Inbound & away** (par 1 — learn arrival+build+depart) and **Block the loads** (par 2 — gather the *loads* A,B,E as the outbound, leave the empties; teaches "don't mix loads & empties" via the win condition). `startState` is captured **post-arrival** so Reset/Watch-optimal don't replay the arrival; departure nulls `auto` so it doesn't fight the auto-player. Verified: 880-replay harness (40×22) all win at par; live arrival + Watch-optimal build + departure ("Train away. Par — 1 joint!"). **Phase 2 (next): require the exact outbound ORDER** (use `ordered:true` on the depart goal + blocking).
...
- **Puzzle set (2026-06-17): 22 puzzles, 6 tiers** (`tier` field, grouped in `#pz`; the 6th tier is **Inbound / outbound**, see its own bullet below): **Basics** (move/dig/spot, two-track), **Digging & room** (buried pairs, 6-car cut that won't fit so you stage it, clear-an-occupied-track-and-rebuild, one-from-each-of-three), **Kicking** (first kick, make-a-kick-track, kick-to-two, long kick), **Yard work** (Reverse it + Build in order = **ordered goals**; Crowded yard; The transfer capstone), and **Hard mode** (2026-06-17): Seven-car dig (par 3), Re-sort four (ordered, par 5 — reordering is coupling-heavy), Eight-car split (8 cars + a kick, par 3), Kicker's special (two tied cuts, par 2). The hard set was solved by the same Dijkstra solver upgraded to the **current** engine (length-based `LEAD_LEN`, foul/`FZONE`, conservative start head) so par/opt respect the lead + foul constraints; all 20 re-verified by the 40×16... now 40×20 = 800-replay harness (every opt wins at par, no foul/lead failure) + a live Watch-optimal run of the 7-move Eight-car split. Goals can be `ordered:true` → win uses `eqArr` (exact slot order) instead of `setEq`. **Every puzzle machine-verified solvable** by a throwaway BFS solver that mirrors the engine (lining ignored since always satisfiable; respects LEAD_CAP, kick legality, securing, and the "all 2+ cuts tied" win rule) — solver deleted after use, rebuild from the model if needed. Strays/scenery use lowercase labels, interactive cars uppercase.
