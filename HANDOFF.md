...
- **Honest rough edges (polish later):** KICK mid-phase animation math is a bit hacky (works, looks fine); spotting onto an already-occupied track snaps existing cars on re-render (throat-anchor); puzzles are dig-out style only so far. Linked from the hub as a **"Game"** card (orange LED, `.tag.g`).
