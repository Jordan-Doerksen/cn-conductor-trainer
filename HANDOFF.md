...
- **Batches** (`GROUPS`): Dig-outs (`#`), Tight room (`C`, capacity-limited), Build outbound (`O`, exact order), Leave it clean (`T`, must empty scratch + lead), plus **Mixed** (random). Nav = batch tabs + numbered chips (cleaner than a row of pills).
