...
- **Sound + mute (2026-06-17):** synthesized Web Audio (no files) — coupling clunk, kick roll (noise+sweep), points click, win chime, refusal buzz; lazy `AudioContext` resumed on first gesture. 🔊/🔇 toggle in the header, persisted in `localStorage` (`gpss_mute`). All sfx verified to fire without error.
